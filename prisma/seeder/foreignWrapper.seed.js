...
const foreign_wrapper = async () => {
  ...
  await prisma.$queryRaw`CREATE SERVER foreigndb_fdw FOREIGN DATA WRAPPER postgres_fdw OPTIONS (host '172.18.1.50', port '5432', dbname 'juidco_masters')`;
  await prisma.$queryRaw`CREATE USER MAPPING FOR postgres SERVER foreigndb_fdw OPTIONS (user 'postgres', password 'Perfect@#@##)@?')`;
  ...
  await prisma.$queryRaw`CREATE USER MAPPING FOR postgres SERVER foreigndb_fdw OPTIONS (user 'postgres', password 'postgres')`;
